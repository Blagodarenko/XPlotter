...
BOOL SetPrivilege(void)
{
	...
	{
		printColouredMessage("The token does not have the specified privilege.\nFor faster writing you should restart plotter with Administrative rights.", RED);
		return FALSE;
	}
	return TRUE;
}
...
void printProgress(unsigned long long percentage)
{
#define PBSTR "**********************************************************"
#define PBWIDTH 60
	...
	printf("\r%3llu%% [%.*s%*s]", percentage, lpad, PBSTR, rpad, "");
	fflush(stdout);
}
...
int main(int argc, char* argv[])
{
	...
	for (size_t i = 1; i < args.size(); i++)
	{
		...
		if (args[i] == "-low")
		{
			...
			{
				printLastError(" Error setting priority 'below normal'");
			}
			else printf("Set priority 'below normal' - Ok\n");
		}
	}
	...
	if (supports_streams == 1)
	{
		...
		{
			printColouredMessage("Error creating stream for file " + out_path + filename, RED);
		}
		else nonces_done = read_from_stream();
	...
	{
		Sleep(100);
		printf("\r[%llu%%] CPU: %llu nonces done                   ", (nonces_done * 100) / nonces, nonces_done + x);
		printf("\t\tHDD: Writing scoops: %.2f%%", (double)(written_scoops * 100) / (double)HASH_CAP);
	}
	printf("\rWait, closing file...                                                                            \n");
	if (writer.joinable()) writer.join();
